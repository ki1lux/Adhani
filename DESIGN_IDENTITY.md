...
| `#5FE08E` | 🔒 | **Success / target-state reached.** Refines `#4DE599` (same role: "you got it"). **Role widened:** this used to say "still not a general confirmation color for arbitrary snackbars or checkmarks." That held while confirmations were three hand-rolled snackbars with no status language at all. Now there is exactly one confirmation component (`AppToast`), so "success" has a single, controlled home rather than being sprinkled per-screen — the discipline is enforced by the component, not by banning the hue. | `QiblaScreen` (aligned with Qibla), `SettingsScreen` (alarm scheduled today / working) |
| `#34B26C` | 🔁 | **Success, darkened for solid fills.** Same hue as `#5FE08E`, dropped in lightness so white content can sit on it — the mint is too light to carry a white glyph. Exists only because `AppToast`'s confirmation badge is a filled square, not a tinted outline. Mirrors the `accent`/`accentDeep` and `danger`/`dangerDeep` pairing rather than introducing a new hue. | `AppToast` success badge |
